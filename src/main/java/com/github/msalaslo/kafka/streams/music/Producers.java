...
public class Producers {
	...
	public static void main(String... args) throws Exception {
		...
		final Map<String, String> serdeConfig = Collections
				.singletonMap(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, ApplicationConfiguration.SCHEMA_REGISTRY_URL);
		...
		final List<Song> songs = Arrays.asList(
				...
				new Song(6L, "The Audacity Of Hype", "Jello Biafra And The Guantanamo School Of " + "Medicine",
						"Three Strikes", "Punk"),
		...
		props.put(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, ApplicationConfiguration.SCHEMA_REGISTRY_URL);
		...
		template1.setDefaultTopic(ApplicationConfiguration.SONG_FEED_INPUT_TOPIC);

		songs.forEach(song -> {
			System.out.println("Writing song information for '" + song.getName() + "' to input topic "
					+ ApplicationConfiguration.SONG_FEED_INPUT_TOPIC);
		...
		template.setDefaultTopic(ApplicationConfiguration.PLAY_EVENTS_INPUT_TOPIC);

		final long duration = 6000 * 1000L;
		final Random random = new Random();

		// send a play event every 1 milliseconds
		while (true) {
			...
			System.out.println("Writing play event for song " + song.getName() + " to input topic "
					+ ApplicationConfiguration.PLAY_EVENTS_INPUT_TOPIC);
